...
DEFAULT_LIST_COMMAND = "plnf list"
...
def formatPluginsList(pluginsDatabase):
    try:
        totalCount = len(pluginsDatabase)
        parts = [f"*List of plugins*\nTotal: {totalCount}"]
        
        for pluginKey, pluginData in pluginsDatabase.items():
            displayName = pluginData.get("displayName", pluginKey)
            link = pluginData.get("link", "")
            
            if link:
                parts.append(f"[{displayName}]({link})")
            else:
                parts.append(displayName)
        
        return "\n".join(parts)
    except Exception as e:
        log(f"Error formatting plugins list: {str(e)}")
...
class PluginFinderPlugin(BasePlugin):
    def __init__(self):
        ...
        self.listCommand = DEFAULT_LIST_COMMAND
    ...
    def on_plugin_load(self):
        ...
        self.listCommand = self.get_setting("list_command", DEFAULT_LIST_COMMAND)
...
• plnf list — show all plugins""")
    ...
    def _processPluginList(self, peerId: Any, replyToMsg: Any, replyToTopMsg: Any):
        ...
        messageContent = formatPluginsList(pluginsDatabase)
        ...
        try:
            ...
            headerLength = len("List of plugins\nTotal: ")
            totalCount = len(pluginsDatabase)
            totalText = str(totalCount)
            blockquoteStart = headerLength + len(totalText) + 1
            ...
            blockquoteEntity.length = len(parsedMessage.text) - blockquoteStart
    ...
    def on_send_message_hook(self, account: int, params: Any) -> HookResult:
        ...
        listCommandPrefix = self.listCommand.strip()
        if messageText == listCommandPrefix or messageText.startswith(listCommandPrefix + " "):
            peerId = params.peer
            replyToMsg = getattr(params, "replyToMsg", None)
            replyToTopMsg = getattr(params, "replyToTopMsg", None)
            
            run_on_queue(lambda: self._processPluginList(peerId, replyToMsg, replyToTopMsg))
            return HookResult(strategy=HookStrategy.CANCEL)
