...
class LocalizationManager:
    ...
    strings = {
        "ru": {
            ...
            "USAGE_SSPAM": "Использование: .sspam <текст с переносами строк>",
        ...
        "en": {
            ...
            "USAGE_SSPAM": "Usage: .sspam <text with line breaks>",
...
class WordSpamPlugin(BasePlugin):
    ...
    def on_send_message_hook(self, account, params):
        ...
        if text.startswith(".sspam"):
            try:
                parts = text.split(maxsplit=1)
                if len(parts) < 2:
                    BulletinHelper.show_error(locali.get_string("USAGE_SSPAM"), get_last_fragment())
                    return HookResult(strategy=HookStrategy.CANCEL, params=params)
                ...
                for idx, line in enumerate(lines):
                    def send_line(l=line, p_id=peer_id):
                        if not global_spam_status.get(p_id, False):
                            return
                        msg_params = {"peer": peer, "message": l}
                        if replyToMsg is not None:
                            msg_params["replyToMsg"] = replyToMsg
                        if replyToTopMsg is not None:
                            msg_params["replyToTopMsg"] = replyToTopMsg
                        send_message(msg_params)
                    run_on_queue(send_line, "pluginsQueue", global_delay * idx)
                return HookResult(strategy=HookStrategy.CANCEL, params=params)
            except Exception:
                BulletinHelper.show_error(locali.get_string("USAGE_SSPAM"), get_last_fragment())
                return HookResult(strategy=HookStrategy.CANCEL, params=params)
